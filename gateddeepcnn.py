...
from keras.layers import BatchNormalization, Add, Multiply, Activation
...
def train_model( use_char = False):
    ...
    if use_char:
        ...
        MAX_LEN = 300
        name = 'gateddeepcnnmodel_term.h5'
    ...
    print('train')
    print('define model')
    model = GatedDeepCNNModel(embed_matrix=embed_matrix, MAX_LEN=MAX_LEN, name= name)
    model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
    del model
    model = GatedDeepCNNModel(embed_matrix=embed_matrix, train_embed=True, MAX_LEN=MAX_LEN, name= name)
    model.load_weights( )
    model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
...
def train_model_pe( use_char = False):
    ...
    print('train')
    print('define model')
    model = GatedDeepCNNModel(embed_matrix=embed_matrix, MAX_LEN=MAX_LEN, name= name, PE=True)
    model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
    del model
    model = GatedDeepCNNModel(embed_matrix=embed_matrix, MAX_LEN=MAX_LEN, name= name, PE=True, train_embed=True)
    model.load_weights()
    model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
