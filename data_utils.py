...
class TrainConfigure(object):
    def __init__(self):
        self.has_label = True
        self.filename = 'training_new.txt'
        self.new_dict = True
        self.char_dict = 'char_dict.pkl'
        self.char_dict_plain = 'char.dict'
        self.char_file = 'data.dict'
        self.term_dict = 'term_dict.pkl'
        self.term_dict_plain = 'term.dict'
        self.term_file = 'data_terms.pkl'
        self.feat_file = 'data_feat.pkl'
        self.feat_norm = 'feat.norm'

        self.new_tfidf = True
        self.tfidf_helper = 'tfidf_helper.pkl'
        self.tfidf_file = 'data_tfidf.pkl'

class ValidConfigure(object):
    def __init__(self):
        self.has_label = False
        self.filename = 'validation_new.txt'
        self.new_dict = False
        self.char_dict = 'char_dict.pkl'
        # self.char_dict_plain = 'char.dict'
        self.char_file = 'data_val.dict'
        self.term_dict = 'term_dict.pkl'
        # self.term_dict_plain = 'term.dict'
        self.term_file = 'data_terms_val.pkl'
        self.feat_file = 'data_feat_val.pkl'
        self.feat_norm = 'feat.norm'
        self.out_file = 'result.csv'

        self.new_tfidf = False
        self.tfidf_helper = 'tfidf_helper.pkl'
        self.tfidf_file = 'data_tfidf_val.pkl'
        self.lda_file = 'data/lda_vec_val.pkl'

class TestConfigure(object):
    def __init__(self):
        self.has_label = False
        self.filename = 'testing.txt'
        self.new_dict = False
        self.char_dict = 'char_dict.pkl'
        # self.char_dict_plain = 'char.dict'
        self.char_file = 'data_test.dict'
        self.term_dict = 'term_dict.pkl'
        # self.term_dict_plain = 'term.dict'
        self.term_file = 'data_terms_test.pkl'
        self.feat_file = 'data_feat_test.pkl'
        self.feat_norm = 'feat.norm'
        self.out_file = 'result_test.csv'

        self.new_tfidf = False
        self.tfidf_helper = 'tfidf_helper.pkl'
        self.tfidf_file = 'data_tfidf_test.pkl'
        self.lda_file = 'data/lda_vec_test.pkl'
...
def convert_char(conf,  V=40000, MAX_LEN=600):
...
def convert_tfidf(conf):
    ...
    if conf.new_tfidf:
        ...
        vectorizer_term = TfidfVectorizer(encoding="utf8", analyzer='word', ngram_range=(gram_low, gram_high),
                                          max_features=max_feature)
        ...
        pickle_dump((vectorizer_char, scaler, vectorizer_term, scaler_term), conf.tfidf_helper)
    else:
        vectorizer_char, scaler, vectorizer_term, scaler_term = pickle_load( conf.tfidf_helper)
        x = vectorizer_char.transform(texts)
        x = scaler.transform(x)
        x_term = vectorizer_term.transform(texts_cut)
        x_term = scaler_term.transform(x_term)
...
def do_shuffle():
    ...
    """
    conf = TrainConfigure()
    data_dict = pickle_load(conf.char_file)
    x = data_dict['x']
    y = data_dict['y']
    xterm = pickle_load(conf.term_file)
    xfeat = pickle_load(conf.feat_file)
    xt = pickle_load('data/lda_vec.pkl')
    x_tfidf, x_term_tfidf = pickle_load(conf.tfidf_file)
    indices = np.arange(x.shape[0])
    np.random.shuffle(indices)
    x, y, xterm, xfeat, xt, x_tfidf,x_term_tfidf = x[indices], y[indices], xterm[indices], xfeat[indices], xt[indices], x_tfidf[indices],x_term_tfidf[indices]
    data_dict['x'] = x
    data_dict['y'] = y
    pickle_dump(data_dict, conf.char_file)
    pickle_dump(xterm, conf.term_file)
    pickle_dump(xfeat, conf.feat_file)
    pickle_dump(xt, 'data/lda_vec.pkl')
    pickle_dump((x_tfidf, x_term_tfidf), conf.tfidf_file)
    print('shuffle done.')
