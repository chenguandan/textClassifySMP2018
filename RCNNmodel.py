...
if __name__ == '__main__':
    ...
    if len(sys.argv)>1 and sys.argv[1] == 'char':
        if len(sys.argv) > 2 and sys.argv[2] == 'pe':
            ...
            print('train')
            model = RCNNModel(MAX_LEN=600, name='RCNNmodel_char_PE.h5', embed_matrix=char_embed_matrix, PE=True)
            ...
            x_tn, y_tn, x_val, y_val = training_utils.split(x_tn, y_tn, shuffle=False)
            print('train')
            model = RCNNModel(MAX_LEN=600, name='RCNNmodel_char.h5', embed_matrix=char_embed_matrix)
            model.train( x_tn, y_tn, x_val, y_val, x_ts, y_ts)
    else:
        if len(sys.argv) > 2 and sys.argv[2] == 'pe':
            ...
            x_tn, y_tn, x_val, y_val = training_utils.split(x_tn, y_tn, shuffle=False)
            print('train')
            model = RCNNModel(MAX_LEN=300, embed_matrix=term_embed_matrix, PE=True, name='RCNNmodel_PE.h5')
            model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
        else:
