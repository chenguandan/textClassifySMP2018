...
class GatedConvTopicModel(object):
    def __init__(self, C = 4, V = 40000, MAX_LEN= 600, embed_matrix = None,
                 name='gatedconvtopicmodel.h5', PE= False, train_embed=False,
                 train_top=True, lr=0.001):
        ...
        if embed_matrix is None:
            ...
            x = embed1(input)
            xt_repeat = RepeatVector(MAX_LEN)(topic_in)
        ...
        z = Dense(C, activation='softmax', trainable=train_top)(z)
        if self.PE:
            model = Model([input, e_input, topic_in], z)
        else:
            model = Model([input, topic_in], z)
        # opt = Adagrad(lr=lr)
        opt = Adam(lr=lr)
...
def train_model( use_char=False):
    ...
    if use_char:
        ...
        MAX_LEN = 300
        name = 'gatedconvtopicmodel_term.h5'
    print('load embed done.')
    y = to_categorical(data_dict['y'])
    xt = data_utils.pickle_load('data/lda_vec.pkl')
    x_tn, y_tn, x_ts, y_ts = training_utils.split([x, xt], y, shuffle=False)
    x_tn, y_tn, x_val, y_val = training_utils.split(x_tn, y_tn, shuffle=False)
    print('train')
    print('define model')
    model = GatedConvTopicModel(embed_matrix=embed_matrix, MAX_LEN=MAX_LEN, name= name)
    model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
    del model
    model = GatedConvTopicModel(embed_matrix=embed_matrix, MAX_LEN=MAX_LEN, name= name, train_embed=True, lr=0.001)
    model.load_weights( )
    model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
...
def train_model_pe( use_char = False):
    ...
    if use_char:
        ...
        embed_matrix = data_utils.load_embedding(vocab_dict,
                                                 ...
                                                 dump_path='data/term_embed.pkl')
        MAX_LEN = 300
        name = 'gatedconvtopicmodel_term_PE.h5'
    print('load embed done.')
    y = to_categorical(data_dict['y'])
    ...
    xe = np.array(xe)
    xt = data_utils.pickle_load('data/lda_vec.pkl')
    x_tn, y_tn, x_ts, y_ts = training_utils.split([x, xe, xt] , y, shuffle=False)
    x_tn, y_tn, x_val, y_val = training_utils.split(x_tn, y_tn, shuffle=False)
    print('train')
    print('define model')
    model = GatedConvTopicModel(embed_matrix=embed_matrix, MAX_LEN=MAX_LEN, name= name, PE=True)
    model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
    del model
    model = GatedConvTopicModel(embed_matrix=embed_matrix, MAX_LEN=MAX_LEN, name= name, PE=True, train_embed=True, train_top=False)
    model.load_weights()
    model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
    del model
    model = GatedConvTopicModel(embed_matrix=embed_matrix, MAX_LEN=MAX_LEN, name= name, PE=True, train_embed=True,
                                train_top=True)
    model.load_weights()
    model.train(x_tn, y_tn, x_val, y_val, x_ts, y_ts)
